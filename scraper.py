...
def has_informative_content(info): 
    ...
    #Threshold but it can be different, just a basic checking of real content # Return true if the page has high textual information content, and return false otherwise
...
# detects repeating directory patterns such as /a/a/a
...
def is_year(s):
    return re.fullmatch(r"(19|20)\d{2}", s)

def is_month(s):
    return re.fullmatch(r"0?[1-9]|1[0-2]", s)

def is_day(s):
    return re.fullmatch(r"0?[1-9]|[12][0-9]|3[01]", s)

def is_date(a, b, c):
    # YYYY/MM/DD
    if is_year(a) and is_month(b) and is_day(c):
        return True

    # MM/DD/YYYY
    if is_month(a) and is_day(b) and is_year(c):
        return True

    # DD/MM/YYYY
    if is_day(a) and is_month(b) and is_year(c):
        return True
...
def has_date_trap(path):
    ...
    for i in range(len(segments) - 2):
        if is_date(segments[i], segments[i + 1], segments[i + 2]):
            prefix = "/".join(segments[:i+2])
            calendar_counts[prefix] += 1
            if calendar_counts[prefix] > 20:
                return False
    return True    
...
def variants_trap(path, query):
    ...
    if 'order' in queries:
        if path in seen_order_paths:
            return False
        seen_order_paths.add(path)
